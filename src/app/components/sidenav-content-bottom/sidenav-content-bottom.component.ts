...
@Component({
  selector: 'app-sidenav-content-bottom',
  templateUrl: '../content/content.component.html',
  styleUrls: ['../content/content.component.scss'],
})
export class SidenavContentBottomComponent extends ContentComponent {
  ...
  changeSize(dimensions) {
    ...
    if (dimensions.height) {
      ...
      if (dimensions.height === 100) {
        this.navConfig.down.action = TransformActions.collapseHeight;
        this.navConfig.up.action = TransformActions.maximizeHeight;
      }
      if (dimensions.height === 40) {
        this.navConfig.up.action = TransformActions.expandHeight;
      }
      this.navConfig.up.disabled = dimensions.height === 260;
